...
public class  ConnectTOBroadcast{
    ...
    public ConnectTOBroadcast( String host, int port, final String message) throws IOException {
        ...
        //try to connect to the server side
        sockChannelToBroadcast.connect( new InetSocketAddress("127.0.0.1", 19998), sockChannelToBroadcast, new CompletionHandler<Void, AsynchronousSocketChannel >() {
            @Override
            public void completed(Void result, AsynchronousSocketChannel channel ) {
                //start to read message
    ...
    public static void main( String...args ) {
        try {
            ...
            //for( int i = 0; i < 100; i++ ) {
            new ConnectTOBroadcast( "127.0.0.1", 19997, "echo test 19 ");
            //}
            for(;;){
